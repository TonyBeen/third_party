...
int main(int argc, char **argv)
{
    ...
    ctx.ares_opts.sock_state_cb = [](void *arg, int fd, int readable, int writable) {
        ...
        if (event_assign(&event, ctx->base, fd, events, [](int fd, short events, void *arg) {
                       int w = ARES_SOCKET_BAD, r = ARES_SOCKET_BAD;
                       auto ctx = reinterpret_cast<context *>(arg);
                       if (events & EV_READ) r = fd;
                       if (events & EV_WRITE) w = fd;
                       ares_process_fd(ctx->channel, r, w); }, ctx) != 0)
        {
    ...
    event_assign(&ctx.timeout_evt, ctx.base, -1, EV_TIMEOUT, [](int fd, short events, void *arg) {
                    (void)fd;
                    (void)events;
                    auto ctx = reinterpret_cast<context *>(arg);
    ...
    ares_gethostbyname(ctx.channel, argv[1], AF_INET, [](void *data, int status, int timeouts, struct hostent *hostent) {
